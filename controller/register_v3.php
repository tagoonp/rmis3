...
$instType = mysqli_real_escape_string($conn, $_POST['institution']);
...
if($query = mysqli_query($conn, $strSQL)){
  ...
  $strSQL = "INSERT INTO userinfo (id_prefix, prefix_th, prefix_en, fname, lname, fname_en, lname_en, id_dept, id_personnel, dept, dept_en, dept_group, expertise, rs_interest, address, tel_mobile, tel_office,  user_id)
            VALUES ('58', '$prefix_th', '$prefix_en', '$fname_th', '$lname_th', '$fname_en', '$lname_en', '19', '$position', '$dept_th', '$dept_en', '$instType', '$exp', '$ri', '$address', '$mobile', '$office', '$rid')
            ";
  if($query = mysqli_query($conn, $strSQL)){

    $strSQL = "UPDATE useraccount SET id_pm = 'PI_0$rid' WHERE id = '$rid'";
    mysqli_query($conn, $strSQL);

    $data['reg_status'] = 'Success';
    $return[] = $data;

  }else{

    $data['reg_status'] = 'Un-success';
    $data['err_message'] = 'Userinfo error';
    $data['err_info'] = $strSQL;
    $return[] = $data;
  ...
  $data['reg_status'] = 'Un-success';
  $data['err_message'] = 'Useraccount error';
  $data['err_info'] = $strSQL;
  $return[] = $data;
