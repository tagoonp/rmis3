...
if($query){
  ...
  while ($row = mysqli_fetch_array($query)) {
    ...
    if($row['rct_type'] == 'Exempt'){
      $buf['ผลการพิจารณา'] = '-';
    }else if($row['rct_type'] == 'Expedited'){
      $buf['ผลการพิจารณา'] = 'Monir';
    }else if(($row['rct_type'] == 'Fullboard (Bio)') || ($row['rct_type'] == 'Fullboard (Social)')){
      $strSQL = "SELECT * FROM research_assign_fullboard_agendar WHERE rafa_id_rs = '".$row['id_rs']."' AND rafa_status = '1' ORDER BY rafa_add_datetime LIMIT 1";
      $query3 = mysqli_query($conn, $strSQL);
      if($query3){
        $row3 = mysqli_fetch_assoc($query3);
        if($row3['rafa_result'] == '3'){
          $buf['ผลการพิจารณา'] = 'Major';
        }else{
          $buf['ผลการพิจารณา'] = 'Minor';
        }
      }else{
        $buf['ผลการพิจารณา'] = 'NA';
      }
