...
if($result){
  ...
  $strSQL = "UPDATE research SET budget = '$budget1' WHERE id_rs = '$id_rs'";
  $result = mysqli_query($conn, $strSQL);
  ...
  if($result){
    ...
    $strSQL = "INSERT INTO log_research (log_activity, log_detail, log_datetime, id_rs, log_by )
              VALUES ('ปรับปรุงงบประมาณตั้งต้น', $budget.' -> '.$budget1, '$datetime', '$id_rs', 'Staff : ".$id."')";
    ...
    VALUES ('Add note', '<p>[System] ปรับปรุงงบประมาณตั้งต้น</p><p>$budget -> $budget1 </p>', '$log_ip', '$datetime', '$id_rs', 'staff', '0', '$id')";
    mysqli_query($conn, $strSQL);
