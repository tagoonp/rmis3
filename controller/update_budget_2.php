...
if($result){
  ...
  $budget = $row['final_budget'];

  $strSQL = "UPDATE research SET final_budget = '$budget1' WHERE id_rs = '$id_rs'";
  $result = mysqli_query($conn, $strSQL);
  ...
  if($result){
    ...
    $strSQL = "INSERT INTO log_research (log_activity, log_detail, log_datetime, id_rs, log_by )
              VALUES ('ปรับปรุงงบประมาณที่ได้รับจริง', $budget.' -> '.$budget1, '$datetime', '$id_rs', 'Staff : ".$id."')";
    ...
    VALUES ('Add note', '<p>[System] ปรับปรุงงบประมาณที่ได้รับจริง</p><p>$budget -> $budget1 </p>', '$log_ip', '$datetime', '$id_rs', 'staff', '0', '$id')";
    mysqli_query($conn, $strSQL);
