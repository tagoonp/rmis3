...
$protocol_number = mysqli_real_escape_string($conn, $_POST['protocol_number']);
...
if($result){
  ...
  $strSQL = "UPDATE research SET protocol_no = '$protocol_number' WHERE id_rs = '$id_rs'";
  $result = mysqli_query($conn, $strSQL);
  ...
  if($result){
    ...
    $strSQL = "INSERT INTO log_research (log_activity, log_detail, log_datetime, id_rs, log_by )
              VALUES ('แก้ไข Protocol number', 'แก้ไข Protocol number เป็น '.$protocol_number, '$datetime', '$id_rs', 'Staff : ".$id."')";
    ...
    VALUES ('Add note', '<p>[System] เจ้าหน้าที่แก้ไข Protocol number เป็น $protocol_number</p>', '$log_ip', '$datetime', '$id_rs', 'staff', '0', '$id')";
    mysqli_query($conn, $strSQL);
