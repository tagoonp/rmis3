...
$frow = mysqli_real_escape_string($conn, $_POST['frow']);
$nlimit = mysqli_real_escape_string($conn, $_POST['nrow']);
    ...
    $strSQL = "SELECT * FROM research a LEFT JOIN type_research b ON a.id_type  = b.id_type
             ...
             LEFT JOIN userinfo g ON f.id = g.user_id
              WHERE
                ...
                AND f.delete_status = '0'
                AND (
                  a.id_rs LIKE '$searchkey%'
                  OR a.code_apdu LIKE '$searchkey%'
                  OR a.title_en LIKE '$searchkey%'
                  OR a.title_th LIKE '$searchkey%'
                  OR g.fname LIKE '$searchkey%'
                  OR g.lname LIKE '$searchkey%'
                )
                ORDER BY a.ord_id
                LIMIT $frow, $nlimit
                ";
