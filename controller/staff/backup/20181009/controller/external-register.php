<?php
include "config.class.php";

if(!isset($_POST['email'])){
  mysqli_close($conn);
  die();
}

if(!isset($_POST['prefix'])){
  mysqli_close($conn);
  die();
}

if(!isset($_POST['fname'])){
  mysqli_close($conn);
  die();
}

if(!isset($_POST['lname'])){
  mysqli_close($conn);
  die();
}

if(!isset($_POST['password'])){
  mysqli_close($conn);
  die();
}

$email = mysqli_real_escape_string($conn, $_POST['email']);
$id_prefix = mysqli_real_escape_string($conn, $_POST['prefix']);
$fname = mysqli_real_escape_string($conn, $_POST['fname']);
$lname = mysqli_real_escape_string($conn, $_POST['lname']);
$position = mysqli_real_escape_string($conn, $_POST['position']);
$instType = mysqli_real_escape_string($conn, $_POST['instType']);
$instName = mysqli_real_escape_string($conn, $_POST['instName']);
$exp = mysqli_real_escape_string($conn, $_POST['exp']);
$ri = mysqli_real_escape_string($conn, $_POST['ri']);
$address = mysqli_real_escape_string($conn, $_POST['address']);
$mobile = mysqli_real_escape_string($conn, $_POST['mobile']);
$office = mysqli_real_escape_string($conn, $_POST['office']);
$password = mysqli_real_escape_string($conn, base64_encode($_POST['password']));
$date = date("Y-m-d H:i:s");
$sid = generateRandomString();
$return = [];

$strSQL = "SELECT * FROM useraccount WHERE email = '$email' AND delete_status = '0' AND active_status = '1'" ;
$query = mysqli_query($conn, $strSQL);
if($query){
  $numrow = mysqli_num_rows($query);
  if($numrow > 0){
    $return['status'] = 'D';
    echo json_encode($return);
    mysqli_close($conn);
    die();
  }
}

$strSQL = "INSERT INTO useraccount (username, password, email, usertype, active_status, allow_status, reg_datetime, SID, pm_role)
          VALUES ('$email', '$password', '$email', 'pm', '0', '0', '$date', '$sid', '1')
          ";
if($query = mysqli_query($conn, $strSQL)){

  $strSQL = "SELECT id FROM useraccount WHERE email = '$email' AND SID = '$sid'";
  $query = mysqli_query($conn, $strSQL);
  $row = mysqli_fetch_assoc($query);

  $dept_info = '';
  if($instType == '1'){
    $dept_info = 'มหาวิทยาลัย ('.$instName.')';
  }else if($instType == '2'){
    $dept_info = 'โรงพยาบาล ('.$instName.')';
  }else{
    $dept_info = 'อื่น ๆ ('.$instName.')';
  }

  $strSQL = "INSERT INTO userinfo (id_prefix, fname, lname, id_dept, id_personnel, dept, expertise, rs_interest, address, tel_mobile, tel_office,  user_id)
            VALUES ('$id_prefix', '$fname', '$lname', '19', '$position', '$dept_info', '$exp', '$ri', '$address', '$mobile', '$office', '".$row['id']."')
            ";
  if($query = mysqli_query($conn, $strSQL)){

    $strSQL = "UPDATE useraccount SET id_pm = 'PI_0".$row['id']."' WHERE id = '".$row['id']."'";
    mysqli_query($conn, $strSQL);

    $strSQL = "SELECT * FROM useraccount a INNER JOIN userinfo  b ON a.email = '$email' AND a.SID = '$sid' ";
    $query = mysqli_query($conn, $strSQL);
    if($query){
      while ($row = mysqli_fetch_array($query)) {
        $buf = [];
        foreach ($row as $key => $value) {
            if(!is_int($key)){
              $buf[$key] = $value;
            }
        }
        $return['data'] = $buf;
        $return['status'] = 'Y';
      }
    }else{
      $return['status'] = $strSQL;
    }


    // mysqli_close($conn);
    // die();
  }else{
    $return['status'] = 'N';
    // mysqli_close($conn);
    // die();
  }
}else{
  $return['status'] = $strSQL;
}


echo json_encode($return);
mysqli_close($conn);
die();

function generateRandomString($length = 20) {
    $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $charactersLength = strlen($characters);
    $randomString = '';
    for ($i = 0; $i < $length; $i++) {
        $randomString .= $characters[rand(0, $charactersLength - 1)];
    }
    return $randomString;
}
