...
if($query){
  ...
  if($nrow > 0){
    ...
    $strSQL = "UPDATE eform_bio_comment SET cfbc_status = '0' WHERE cfbc_efb_id = '".$data['efb_id']."' AND cfbc_element = '$element'" ;
              mysqli_query($conn, $strSQL);

    $strSQL = "INSERT INTO eform_bio_comment (cfbc_comment, cfbc_element, cfbc_efb_id, cfbc_udate)
              VALUES ('$comment', '$element', '".$data['efb_id']."', '$date')
              ";
    $result = mysqli_query($conn, $strSQL);
    if($result){
      $strSQL = "UPDATE eform_bio SET efb_".$element." = '0' WHERE efb_reviewer_id = '$id_reviewer' AND efb_id_rs= '$id_rs'";
      mysqli_query($conn, $strSQL);
    }
  }
