...
$session_id = mysqli_real_escape_string($conn, $_POST['doc_sesion']);
...
if($query = mysqli_query($conn, $strSQL)){
  ...
  $strSQL = "INSERT INTO research_expedited_info (rai_id_rs, rai_staff_id, rai_staff_udate, rai_staff_conf, rai_full_content, rai_main_content, rai_ref, rai_report_round, rai_lang, rai_round_meeting, rai_status, rai_doc_session_id)
            VALUES ('$id_rs', '$id', '$date', '1', '$full_content', '$main_content', '$ref', '$report_round', '$lang', '$atime', '1', '$session_id')";
  $query = mysqli_query($conn, $strSQL);
  ...
  $strSQL = "INSERT INTO log_research (log_activity, log_detail, log_datetime, id_rs, log_by ) VALUES ('Wait approval from chairman', 'เจ้าหน้าที่ส่งเลขาตรวจสอบโครงการ Expedited', '$date', '$id_rs', 'Staff : ".$id."')";
  mysqli_query($conn, $strSQL);

  $strSQL = "INSERT INTO log_note (log_activity, log_detail, log_ip, log_datetime, log_id_rs, log_by_role, log_by_id ) VALUES ('Add note', '<p>[System] ส่งแบบร่างใบรับรองเเพื่อให้เลขา EC ตรวจสอบ</p>', '$ip_add', '$date', '$id_rs', 'staff', '$id')";
  mysqli_query($conn, $strSQL);
