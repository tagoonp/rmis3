...
if($query){
  ...
  while ($row = mysqli_fetch_array($query)) {
    ...
    if($row['delete_flag'] == 'Y'){
      $buf['สถานะโครงการ'] = 'ถอนโครงการ';
    }
    $buf['ประเภทการพิจารณา'] = $row['rct_type'];


    $buf['จำนวนครั้งเอกสารไม่ถูกต้อง'] = $row['ep'] - 1;
...
// header("Content-Disposition:attachment;filename=export-4-62-".date('U').".csv");
// // fputcsv($output, array("ลำดับที่", 'เลขที่โครงการ',	'ปี',	'นักวิจัย',	'ภาควิชา/หน่วยงาน',	'ชื่อโครงการ (ภาษาไทย)',	'ชื่อโครงการ (ภาษาอังกฤษ)',	'คำสำคัญ (ภาษาไทย)',	'คำสำคัญ (ภาษาอังกฤษ)',	'วันที่เริ่มต้น',
// // 	 'วันที่สิ้นสุด',	'สัดส่วนการทดลอง',	'จำนวนผู้ร่วมทดลอง',	'ผู้ร่วมทดลอง',	'งบประมาณทั้งโครงการ',	'กลุ่มแหล่งทุน',	'ชื่อแหล่งทุน',	'สถานะโครงการ',	'ประเภทการพิจารณา',	'จำนวนครั้งเอกสารไม่ถูกต้อง',	'จำนวนการตีกลับขอเอกสาร',	'จำนวนการตีกลับอื่นๆ'));
