                ...
                .always(function(snap){
                  ...
                  if((snap != '') && (snap.length > 0)){
                    ...
                    snap.forEach(function(i){
                      // console.log(i);
                      if(i.riwc_part == 1){
                        a1 += '<p class="" style="padding-bottom: 10px;">' + i.riwc_q + '<hr style="margin-bottom: 5px; border-top: 1px solid #f2f2f2;"></p>';
                      }
                      ...
                      if(i.riwc_part == 2){
                        ...
                        a2 += '<tr>' +
                                ...
                                '<td class="' + $bClass + '"><div>ผู้เชี่ยวชาญอิสระ : <span class="text-success">' + i.rir_binding_label + '</span></div>' +
                                '<div style="border: dashed; border-width: 0px 0px 1px 0px; border-color: #ccc; padding-bottom: 10px;">ประเด็น : <span class="q_id_' + i.riwc_id +'">-</span></div>' + i.riwc_q + '<div class="pt-10"><button class="btn btn-success btn-md  pl-10 pr-10"   data-toggle="modal" data-target=".bs-example-modal-lg-ans"  onclick="setEContent(\'' + i.riwc_id  + '\')"><i class="zmdi zmdi-edit"></i> ตอบคำถาม</button></div>' +
                                '</td>' +
                              '</tr>';

                        getQuestionTitle(i.riwc_id, i.riwc_iform)
...
function getQuestionTitle(id, form){
  if(form == 'bio'){
    var param = {
      q_id: id
    }
    var jxr = $.post(ws_url + 'controller/get_q_title_bio.php', param, function(){}, 'json')
               .always(function(snap){
                 console.log(snap);
                 if((snap != '') && (snap.length > 0)){
                   snap.forEach(i=>{
    ...
    var jxr = $.post(ws_url + 'controller/get_q_title_social.php', param, function(){}, 'json')
               .always(function(snap){
                 console.log(snap);
                 if((snap != '') && (snap.length > 0)){
                   snap.forEach(i=>{
    ...
    var jxr = $.post(ws_url + 'controller/get_q_title_icf.php', param, function(){}, 'json')
               .always(function(snap){
                 console.log(snap);
                 if((snap != '') && (snap.length > 0)){
                   snap.forEach(i=>{
                     $('.q_id_' + id).html(i.cfic_title)
                   })
                 }
               })
  }
