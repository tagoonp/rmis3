var rw_comment = {
  getTitle: function(id, form){
    ...
    if(form == 'bio'){
      ...
      var jxr = $.post(ws_url + 'controller/get_q_title.php', param, function(){}, 'json')
                 .always(function(snap){
                   console.log(snap);
                   if((snap != '') && (snap.length > 0)){
                     snap.forEach(i=>{
                       $('.q_id_' + id).html(i.cfbc_title)
                     })
                   }
                 })
    }
                ...
                .always(function(snap){
                  ...
                  if(j == 1){
                    if((snap != '') && (snap.length > 0)){
                      $('#p1').empty()
                      snap.forEach( i => {
                        contentData = '<div>ข้อคำถาม/ข้อเสนอแนะจากผู้เชี่ยวชาญอิสระ : <span class="text-success">' + i.rir_status + ' (' + i.rir_binding_label + ')</span></div>' +
                        i.riwc_q + '<div class="pt-10 pb-20"><button class="btn btn-success btn-xs  pl-10 pr-10" data-toggle="modal" data-target=".bs-example-modal-lg-1-2" onclick="setEContent(1, \'' + i.riwc_id  + '\')"><i class="zmdi zmdi-edit"></i> แก้ไข</button> <button class="btn btn-info btn-xs  pl-10 pr-10"  onclick="deleteQ(\'' + i.riwc_id + '\')"><i class="zmdi zmdi-delete"></i> ลบ</button></div>';
                  ...
                  // Part 2 - 5
                    ...
                    if((snap != '') && (snap.length > 0)){
                      ...
                      snap.forEach( i => {
                        ...
                        contentData = '<tr>' +
                                '<td class="f500 col-sm-3">ข้อที่ ' + $c + ' - คำถามหรือข้อเสนอแนะ : </td>' +
                                '<td>' + '<div>ผู้เชี่ยวชาญอิสระ : <span class="text-success">' + i.rir_status + ' (' + i.rir_binding_label + ')</span></div><div style="border: dashed; border-width: 0px 0px 1px 0px; border-color: #ccc; padding-bottom: 10px; font-weight: 500; color: #000;">ประเด็น : <span class="q_id_' + i.riwc_id +'">NA</span></div>' + i.riwc_q + '<div class="pt-10"><button class="btn btn-success btn-xs  pl-10 pr-10"   data-toggle="modal" data-target=".bs-example-modal-lg-' + j + '-2"  onclick="setEContent( \'' + j + '\', \'' + i.riwc_id  + '\')"><i class="zmdi zmdi-edit"></i> แก้ไข</button> ' +
                        ...
                        if(j == 4){
                          contentData = '<tr>' +
                                  '<td class="f500 col-sm-3">ข้อที่ ' + $c + ' - คำถามหรือข้อเสนอแนะ : </td>' +
                                  '<td>' + '<div>ผู้เชี่ยวชาญอิสระ : <span class="text-success">' + i.riwc_extra_label + '</span></div><div style="border: dashed; border-width: 0px 0px 1px 0px; border-color: #ccc; padding-bottom: 10px; font-weight: 500; color: #000;">ประเด็น : <span class="q_id_' + i.riwc_id +'">NA</span></div>' + i.riwc_q + '<div class="pt-10"><button class="btn btn-success btn-xs  pl-10 pr-10"   data-toggle="modal" data-target=".bs-example-modal-lg-' + j + '-2"  onclick="setEContent( \'' + j + '\', \'' + i.riwc_id  + '\')"><i class="zmdi zmdi-edit"></i> แก้ไข</button> ' +
                                  '<button class="btn btn-success btn-xs  pl-10 pr-10" ' + $disabled_p + ' onclick="setMoveUp(\'' + i.riwc_id  + '\', \'' + prev_seq + '\', \'' + i.riwc_seq + '\', \'' + j + '\')"><i class="zmdi zmdi-chevron-up"></i> </button> ' +
                                  '<button class="btn btn-success btn-xs  pl-10 pr-10" ' + $disabled_n + ' onclick="setMoveDown(\'' + i.riwc_id  + '\', \'' + next_seq + '\', \'' + i.riwc_seq + '\', \'' + j + '\')"><i class="zmdi zmdi-chevron-down"></i> </button> ' +
                                  '<button class="btn btn-info btn-xs  pl-10 pr-10"  onclick="deleteQ(\'' + i.riwc_id + '\')"><i class="zmdi zmdi-delete"></i> ลบ</button></div>' +
                                  '</td>' +
                        ...
                        rw_comment.getTitle(i.riwc_id, i.riwc_iform)
                        $c++
                      })
                    }
                  }
                  ...
                  if((snap != '') && (snap.length > 0)){
                      ...
                      if((snap != '') && (snap.length > 0)){
                        snap.forEach( i => {
                          ...
                          if(i.rir_binding_label == null){
                            $label = 'ความคิดเห็นจากที่ประชุม'
                          }

                          contentData = '<div>ข้อคำถาม/ข้อเสนอแนะจากผู้เชี่ยวชาญอิสระ : <span class="text-success">' + $label + '</span></div>' + i.riwc_q + '<div class="pt-10 pb-20"><button class="btn btn-success btn-xs  pl-10 pr-10" data-toggle="modal" data-target=".bs-example-modal-lg-1-2" onclick="setEContent(1, \'' + i.riwc_id  + '\')"><i class="zmdi zmdi-edit"></i> แก้ไข</button> <button class="btn btn-info btn-xs  pl-10 pr-10"  onclick="deleteQ(\'' + i.riwc_id + '\')"><i class="zmdi zmdi-delete"></i> ลบ</button></div>';
                          $('#p1').append(contentData)
                        })
                      }
                    }
                    ...
                    snap.forEach(function(i){
                      if(j == 1){
                        // contentData = '<div>ผู้เชี่ยวชาญอิสระ : <span class="text-success">' + i.rir_status + '</span></div>' + i.riwc_q + '<div class="pt-10 pb-20"><button class="btn btn-success btn-xs  pl-10 pr-10" data-toggle="modal" data-target=".bs-example-modal-lg-1-2" onclick="setEContent(1, \'' + i.riwc_id  + '\')"><i class="zmdi zmdi-edit"></i> แก้ไข</button> <button class="btn btn-info btn-xs  pl-10 pr-10"  onclick="deleteQ(\'' + i.riwc_id + '\')"><i class="zmdi zmdi-delete"></i> ลบ</button></div>';
                        // $('#p1').append(contentData)
                      }else if((j == 2) || (j == 3) || (j == 4)){
