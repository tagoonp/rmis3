  ...
  save_note: function(){
    ...
    if(note_data == ''){
      // alert('กรุณาเพิ่มเนื้อหา/บันทึกก่อน')
			try {
				swal("ขออภัย!", "กรุณาเพิ่มเนื้อหา/บันทึกก่อน!", "error")
			} catch (e) {
				alert('กรุณาเพิ่มเนื้อหา/บันทึกก่อน')
			} finally {
      ...
      return ;
		...
		var cus = $('#txtNoteOwnerID').val()

		if($('#txtNoteOwnerID').val() == null){
			cus = current_user
		}
                     ...
                     $row = '<tr>' +
                      ...
                      $('.noteSpan').append($row)
											...
											$row2 = '<tr>' +
																'<td>' +
																	'<div class="row fz08">' +
																		'<div class="col-sm-3">' + main_app.convertThaidatetime(i.log_datetime) + '</div>' +
																		'<div class="col-sm-6 text-success">' + i.log_detail + '</div>' +
																		'<div class="col-sm-3 text-success-2">Role ' + i.log_by_role + ' >> ID : ' + i.log_by_id + '<br>' + i.fname + ' ' + i.lname + '</div>' +
																	'</div>' +
																'</td>' +
															'</tr>'
											$('#note_table_1').append($row2)
