          ...
          <div class="col-sm-12">
            ...
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full thfont f14">
              ...
              <tbody>
                ...
                INNER JOIN submit_feedback d on r.id_rs = d.id_rs
                where d.id_reviewer = '".$_SESSION['id']."'
                AND d.sending_flag = 'Y'
                AND d.status_review = '0'
                ORDER BY d.date_submit DESC";
                ...
                if($resultResearch){
                  foreach ($resultResearch as $value) {
                    $strDate = $value["date_submit"];
                    ?>
                    <tr>
                        <td class="text-center" style="vertical-align: top;">
                          <?php
                          if($value['code_apdu']==''){
                            echo $value["year_name"]."-".$value["ord_id"]."-".$value["id_dept"]."-".$value["id_personnel"];
                            ?>
                            <br>
                            <span data-toggle="popover" title="รหัสโครงการ <?php echo "REC. "; echo $value["year_name"];?>-<?php  echo $value["ord_id"];?>-<?php  echo $value["id_dept"];?>-<?php  echo $value["id_personnel"]; ?>" data-placement="top" data-content="รหัสโครงการนี้ออกโดยระบบ RMIS ซึ่งต้องรอการยืนยันรัหสจากสำนักงานจริยธรรมการวิจัยในมนุษย์อีกครั้ง" class="text-red">[ ออกโดยระบบ ]</span>
                            <?php
                          }else{
                            echo $value['code_apdu'];
                            ?>
                            <br>
                            <span data-toggle="popover" title="รหัสโครงการ <?php echo "REC. ".$value['code_apdu']; ?>" data-placement="top" data-content="รหัสโครงการนี้ออกโดยสำนักงานจริยธรรมการวิจัยในมนุษย์และได้รับการยืนยันความถูกต้องแล้ว" class="text-blue">[ ออกโดย REC ]</span>
                            <?php
                          }
                          ?>
                        </td>
                        ...
                        <td class="hidden-xs text-center" style="vertical-align: top;">
                          <font color="red">
                          <?php
                          if($value['sendding_status']=='N'){
                            echo "รอการยืนยันการส่ง";
                          }else{
                            if($resultDM){
                              echo "รอเข้าวาระประชุม";
                            }else{
                              echo $value["status_name"]."</font>";
                            }
                          }
                          ...
                          if($status==1){
                            ...
                            if($value['status_review']==3){
                              ?>
                              <a href="#" class="text-red"><i class="fa fa-search"></i> ขอไม่พิจารณา</a>
                              <?php
                            }else{
                              ?>
                              <a href="comment_update.php?id_rs=<?php echo $value["id_rs"];?>"><i class="fa fa-check"></i> พิจารณา</a>
                              <?php
                            }
                            ?>
                            )
