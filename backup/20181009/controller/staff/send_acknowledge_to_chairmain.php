...
$doc_no = mysqli_real_escape_string($conn, $_POST['doc_no']);
...
if($query = mysqli_query($conn, $strSQL)){
  ...
  $strSQL = "UPDATE research_acknowledge_info SET rai_status = '0' WHERE rai_id_rs = '$id_rs'";
  $query = mysqli_query($conn, $strSQL);

  $strSQL = "INSERT INTO research_acknowledge_info (rai_id_rs, rai_staff_id, rai_staff_udate, rai_staff_conf, rai_full_content, rai_main_content, rai_ref, rai_doc_no, rai_lang, rai_round_meeting, rai_status)
            VALUES ('$id_rs', '$id', '$date', '1', '$full_content', '$main_content', '$ref', '$doc_no', '$lang', '$atime', '1')";
  $query = mysqli_query($conn, $strSQL);
  ...
  $strSQL = "INSERT INTO log_research (log_activity, log_detail, log_datetime, id_rs, log_by ) VALUES ('Wait approval from chairman', 'เจ้าหน้าที่ส่งประธานลงนามโครงการ Exempt', '$date', '$id_rs', 'Staff : ".$id."')";
  mysqli_query($conn, $strSQL);
