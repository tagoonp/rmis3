...
$neworder = 1;
// $strSQL = "SELECT max(ord_id) mord FROM research WHERE id_year = '$year'";
// $query = mysqli_query($conn, $strSQL);
// if($query){
//     while ($row = mysqli_fetch_array($query)) {
//       $neworder = $row['mord'] + 1;
...
// $strSQL = "SELECT * FROM research WHERE id_year = '$year' AND ord_id != '000' AND code_apdu != '' AND research_status = 'new' ORDER BY ord_id DESC";
// $query = mysqli_query($conn, $strSQL);
// $ordIdOnly = '';
// if($query){
//     while ($row = mysqli_fetch_array($query)) {
...
if($query = mysqli_query($conn, $strSQL)){
  ...
  if($rn > 0){
    ...
    while ($row = mysqli_fetch_array($query)) {
      ...
      $strSQL = "INSERT INTO log_note (log_activity, log_detail, log_ip, log_datetime, log_id_rs, log_by_role, log_by_id ) VALUES ('Add note', '<p>เอกสารถูกต้อง</p>".$info."', '$ip_add', '$date', '$id_rs', 'staff', '$id')";
      ...
      $strSQL = "INSERT INTO log_notification (log_activity, log_detail, log_datetime, user_id ) VALUES ('update_pm_status_by_staff', 'เจ้าหน้าที่ทำการปรับปรุงข้อมูลหน่วยงาน/ประเภทบุคคล', '".date('Y-m-d H:i:s')."', '".$row['id']."')";
      mysqli_query($conn, $strSQL);

      $strSQL = "INSERT INTO log_notification (log_activity, log_detail, log_datetime, user_id ) VALUES ('assign_rec_id', 'เจ้าหน้าที่กำหนดรหัสโครงการวิจัย (REC.".$year_code."-".$neworder."-".$dept."-".$ptype.")', '".date('Y-m-d H:i:s')."', '".$row['id']."')";
      mysqli_query($conn, $strSQL);
      ...
      $strSQL = "INSERT INTO log_timeline (lt_datetime, lt_id_rs, lt_from_role, lt_to_role, lt_p1, lt_p2, lt_info )
                 VALUES ('".date('Y-m-d H:i:s')."', '$id_rs', 'staff', 'ec', '$id', '$ec', 'เจ้าหน้าที่ได้เพิ่มผลการตรวจสอบเอกสาร (เอกสารถูกต้องถูกต้อง)')";
      mysqli_query($conn, $strSQL);
