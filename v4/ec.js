...
var assesment_e_form_id = ['2', '3', '4', '7', '13', '8'];
...
var ec = {
  ...
  loadNewInit_by_year: function(){
    ...
    var jxhr = $.post(ws_url + 'controller/ec/check-new-init-by-year-v1.php', param, function(){},'json')
                 .always(function(snap){
                   // console.log(snap);
                   var dt = $('#datable_1').dataTable().api();
                   if((snap!='') && (snap.length > 0)){
                                  ...
                                  if(advisor == true){
                                    ...
                                    if(lang == 'en'){
                                      $('.subPiDiv').append('<div class="row pb-10">' +
                                        ...
                                        '<div class="col-5"><span class="f500">Affiliation :</span> <span id="txtPiDept" class="txt-danger">' + i.co_dept + '</span> <i class="zmdi zmdi-edit" data-toggle="modal" data-target=".bs-example-modal-lg-copi" onclick="setCopiInfo(' + i.copi_id + ')"></i></div>' +
                                      '</div>')
                                    }else{
                                      $('.subPiDiv').append('<div class="row pb-10">' +
                                  ...
                                  var new_copi_list = '<tr>' +
                                                        '<td>' +
                                                          '<div class="row">' +
                                                            ...
                                                            '<div class="col-sm-4">' + i.co_fname + ' ' + i.co_lname + '</div>' +
                                                            '<div class="col-sm-1">' + i.co_ratio + '</div>' +
                                                            '<div class="col-sm-3">' + i.co_job + '</div>' +
                                                            '<div class="col-sm-3 text-right">' +
                                                              '<button class="btn btn-sm btn-square btn-success cp mr-5"><i class="fas fa-pencil-alt"></i></button>' +
                                                              '<button class="btn btn-sm btn-square btn-success cp mr-5"><i class="fas fa-search"></i></button>' +
                    ...
                    if(i.rw_sending_status == '0'){
                      ...
                      $buffer = '<tr>' +
                                   ...
                                   '<td>' +
                                     ...
                                     '</div>' +
                                     '<div id="file_send_id_' + $i + '"></div>' +
                                   '</td>' +
                                  '<tr>';
                      $('#rw_table').append($buffer)
                      loadCHeckfile(i.rir_id, $i, i.id, pname)
                      ...
                      loadSendingFile($i, i.rir_id_reviewer)
                    }
...
function loadSendingFile(ord_id, id_rw){
  var param = {
    id: id_rw,
    id_rs: current_rs_id
  }
                 ...
                 if((snap2 != '') && (snap2.length > 0)){
                   $('ul#ft_file' + ord).empty()
                    snap2.forEach(function(j){
                      var data = '<li class="mb-5">' + j.fid_name + '</li>';
                      if(assesment_e_form_id.indexOf(j.rif_fileid) >= 0){
                        // data = '<li class="mb-5">' + j.fid_name + ' <span id="eform_result_span" class="text-success cp" onclick="viewEform(\'' + j.fid + '\', \'' + id_reviewer + '\', \'' + reviewer_name + '\')">[<i class="fas fa-search"></i> ดูผลการประเมิน]</span></li>';
                        data = '<li class="mb-5"> ' + j.fid_name + ' <a href="Javascript: viewEform(\'' + j.fid + '\', \'' + id_reviewer + '\', \'' + reviewer_name + '\')" ><i class="fa fa-search text-info mr-5"></i></a></li>';
                      }
